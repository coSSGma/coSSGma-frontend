...
export default function Layout() {
  ...
  return (
    <div className={`h-screen flex flex-col`}>
      {location.pathname === "/" ? (
        <header
          className={`${bgColor} h-[129px] flex items-end border-[#D9E3E0]`}
        >
          <p className="text-[25px] font-extrabold px-20 py-10 text-[#1E624D] font-ohsquare">
            브루디
          </p>
        </header>
      ) : (
        <header
          className={`${bgColor} h-[129px] flex justify-center items-end border-b-[3.5px] border-[#D9E3E0]`}
        >
          <div className="w-full h-50 flex justify-between">
            <span className="ml-30 mt-5" onClick={handleNavigate}>
              <img src={backIcon} alt="뒤로가기" width="11" height="25" />
            </span>
            <p className={`text-[18px] font-extrabold ${textColor}`}>
              톡스터디
            </p>
            <span className="mr-[15px] mt-5">
              <img src={settingIcon} alt="설정" width="26" height="26" />
            </span>
          </div>
        </header>
      )}
